...
public class LoopView extends View {
    ...
    private void initLoopView(Context context) {
        textSize = 0;
        colorGray = 0xffa7a7a7; //0xffafafaf;灰
        colorBlack = 0xff313131;// 0xff313131;黑
        colorGrayLight = 0xffdcdcdc; //0xffc5c5c5;灰亮
        l = 2.0F;
        isLoop = true;
        positon = -1;
        r = 9;
    ...
    private void d() {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 11) {//判断是否为平板
            setLayerType(1, null);
        }

        //初始化手势识别类
        gestureDetector = new GestureDetector(context, simpleOnGestureListener);
        gestureDetector.setIsLongpressEnabled(false);//设置关闭长按手势
    ...
    protected void onMeasure(int i1, int j1) {
        setCyclic(true);
        d();
        setMeasuredDimension(v, s);//用来存储测量的宽  高
    }
