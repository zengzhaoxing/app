...
public class UrlCollectPresenter {
    ...
    public void addCollect(UrlBean urlBean) {
        ...
        if (qb.list().size() == 0) {
            App.getDaoSession().insert(urlBean.copy());
    ...
    public void upDataHistory(UrlBean urlBean) {
        QueryBuilder<UrlBean> qb =  App.getDaoSession().queryBuilder(UrlBean.class)
                .where(UrlBeanDao.Properties.Time.eq(urlBean.getTime()),UrlBeanDao.Properties.IsCollect.eq(false));
        List<UrlBean> list = qb.list();
        if (list.size() > 0) {
            delete(list.get(0));
        }
        App.getDaoSession().insert(urlBean);
    }
